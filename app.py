...
if st.button("Predict") and extracted_features:
    if selected_models:
        ...
        for model_name, model in selected_models:
            try:
                if hasattr(model, "predict_proba"):  # For models that predict probabilities (e.g., Random Forest, XGBoost)
                    prediction_probs = model.predict_proba(feature_values)[:, 1]  # Get probability for class 1 (Safe)
                ...
                if model_name == "Neural Networks":
                    prediction_probs = model.predict(feature_values)  # Continuous probabilities
                    prediction_class = convert_to_class_labels(prediction_probs)  # Convert to binary labels
